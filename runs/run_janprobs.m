...
%if ~isfield(sdata,'find_targets')
    ...
    nb_target = nan(4,nb_ttrials);
    nb_nontgt = nan(4,nb_ttrials);
    fprintf(['\n    subject..']);
...
%if ~isfield(sdata,'reorder')
    ...
    % for each subject
    fprintf(['\n    subject..']);
    submat=unique(sdata.sub);  % submat
    for s = 1:length(submat);
        fprintf([num2str(submat(s)),', ']);
        % for each block2
        for b = 1:48;
            % indexes for this s/b
            indx = find(sdata.sub==submat(s) & sdata.block==b);
            ...
            % kind of block
                ...
                %  3 == CF
                %  4 == FC
            blockcode = sdata.blockcode(indx);
            ...
            % reorder
            nb_target(1:4,indx) = nb_target(attside,indx);
            nb_nontgt(1:4,indx) = nb_nontgt(attside,indx);
        end
    end
%else
%    fprintf(['\n    skipping']);
...
%if ~isfield(sdata,'probabilities')
    ...
    for i_subject = 1:nb_subjects
        ...
        for i_novelty = 1:nb_novelties
            ...
            cht = tgtchoice(ii_index);
            chn = tgtchoice(ii_index);
...
%% PLOT ===================================================================
    ...
    for i_novelty = 1:nb_novelties
        for i_side = 1:nb_sides
            ...
            values = nan(nb_subjects,nb_tnts,nb_counts);
            for i_tnt = 1:nb_tnts
                values(:,i_tnt,:) = squeeze(probs(:,i_novelty,i_side,i_tnt,:));
            end
            % plot
            switch i_novelty
                case 1
