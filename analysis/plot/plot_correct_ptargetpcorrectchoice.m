...
function plot_correct_ptargetpcorrectchoice(u_model)
    ...
    for i_novel = 1:nb_novel
        ...
        for i_model = 1:nb_model
            ...
            for i_choice = 1:nb_choice
                ...
                % loop (subject, trial)
                correct = nan(nb_subject,nb_pcorrect,nb_ptarget);
                for i_ptarget = 1:nb_ptarget
                    for i_pcorrect = 1:nb_pcorrect
                        for i_subject = 1:nb_subject
                            ...
                            % value
                            correct(i_subject,i_pcorrect,i_ptarget) = mean(model.correct(ii_resp & ii_trial & ii_novel & ii_choice & ii_subject & ii_pcorrect & ii_ptarget));
                        end
                    end
                end

                % plot
                y = squeeze(nanmean(correct));
                e = squeeze(tools_ste(correct));
