...
%% get values
fiterr = nan(numbers.nb_novelties, numbers.nb_subjects , numbers.nb_propangs , numbers.nb_threshs);
    ...
    for i_subject = 1:numbers.nb_subjects
        ...
        for i_propang = 1:numbers.nb_propangs
            for i_thresh = 1:numbers.nb_threshs
                ...
                thresh  = i_thresh;
                % get value
                %fiterr(i_novelty,i_subject,i_propang,i_thresh) = mean(sdata.modcr(subject,novelty,propabs,propang,thresh,:));
                fiterr(i_novelty,i_subject,i_propang,i_thresh) = sdata.minfits(subject,novelty,propabs,propang,thresh);
            end
        end
    end
end
...
for i_novelty = 1:numbers.nb_novelties
    ...
    % get value
    landscape = squeeze(fiterr(i_novelty,:,:,:));
