...
for i_trial = 1:nb_trial
    ...
    % step
    if (~target);  alpha_t = (1 - model.alpha_rt); end
    if ( target);  alpha_t = (    model.alpha_rt); end
    if (~correct); alpha_c = (1 - model.alpha_rc); end
    if ( correct); alpha_c = (    model.alpha_rc); end
    alpha = model.alpha_m * alpha_t * alpha_c;
