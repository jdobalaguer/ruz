...
mdata=mean(data);
stedata=std(data,[],1)./sqrt(size(data,1));
stddata=std(data,[],1);
...
if length(size_data)>2
    ...
    stddata = reshape(stddata,size_data(2:end));
end
...
%         end
        ...
        pex=p+ex;
      %  line([pex,pex],[mdata(n,p)-(stddata(n,p)),mdata(n,p)+(stddata(n,p))],'color','k','linewidth',2);
        line([pex,pex],[mdata(n,p)-(stedata(n,p)),mdata(n,p)+(stedata(n,p))],'color','k','linewidth',2);
