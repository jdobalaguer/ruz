...
function ratio2_batch_sdata(u_alpham,u_alphar,u_tau)
    %% load
    sdata     = struct();
    models    = struct();
    numbers   = struct();
    ...
    greed_bic = [];
    load('data/sdata.mat');
    load('data/models_ratio2.mat','mdata','greed_bic');
    ...
    %% sdata
    % initialise
    models.ratio2.choice  = nan(size(models.human.choice));
    models.ratio2.correct = nan(size(models.human.correct));
    ...
    for i_novel   = 1:nb_novel
        ...
        % values
        model = mdata(key);
        models.ratio2.choice(ii_frame)  = model.choice(ii_frame);
        models.ratio2.correct(ii_frame) = model.correct(ii_frame);
    ...
    % degrees of freedom
    models.ratio2.df       = 3;
    models.ratio2.fittings = fittings;
