...
function ratio2_batch_run(u_alpham,u_alphar,u_tau)
    %% load
    sdata   = struct();
    numbers = struct();
    mdata   = dict();
    load('data/sdata.mat','sdata','numbers');
    if exist('data/models_ratio2.mat','file')
        load('data/models_ratio2.mat','mdata');
    else
        mbic  = dict();
        save('-v7.3','data/models_ratio2.mat','mdata','mbic');
        clear mbic;
    end
    ...
    if ~exist('greed_bic','var') || nb_loop ~= numel(greed_bic)
        ...
        %% save
        save('data/models_ratio2.mat','-append','mdata');
    else
        %% skip
