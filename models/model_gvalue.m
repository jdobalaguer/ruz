function data = model_gvalue(codez,attside,target,k,prop_abs,prop_ang,gvalue)
...
gminaVcAS   = nan(nb_trials,1);
gmaxaVcAS   = nan(nb_trials,1);
...
for i=1:nb_trials
    ...
    % combination rule
     ...
     % gain factor
     exp_gvalue = exp(gvalue);
     gminaVcAS(i) = min(aVcAS(i,1:k))/exp_gvalue;
     gmaxaVcAS(i) = max(aVcAS(i,1:k))*exp_gvalue;
     
     % policy
     if abs(gminaVcAS(i)) > abs(gmaxaVcAS(i))
         mmaVcAS(i)     = gminaVcAS(i);
     else
         mmaVcAS(i)     = gmaxaVcAS(i);
     end
...
data.gminaVcAS      = gminaVcAS;
data.gmaxaVcAS      = gmaxaVcAS;
