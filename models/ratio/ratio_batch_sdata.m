...
function ratio_batch_sdata(u_alpham,u_alphar,u_tau)
    %% load
    sdata     = struct();
    models    = struct();
    numbers   = struct();
    ...
    greed_bic = [];
    load('data/sdata.mat');
    load('data/models_ratio.mat','mdata','greed_bic');
    ...
    %% sdata
    % initialise
    models.ratio.choice  = nan(size(models.human.choice));
    models.ratio.correct = nan(size(models.human.correct));
    ...
    for i_novel   = 1:nb_novel
        ...
        % values
        model = mdata(key);
        models.ratio.choice(ii_frame)  = model.choice(ii_frame);
        models.ratio.correct(ii_frame) = model.correct(ii_frame);
    ...
    % degrees of freedom
    models.ratio.df       = 3;
    models.ratio.fittings = fittings;
